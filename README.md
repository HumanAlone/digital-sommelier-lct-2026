...
> **Статус:** интерфейс и локальный каталог работают. Распознавание по фотографии, серверная авторизация и оценка качества поиска ждут интеграции с бэкендом и ML. Ниже описаны и готовая часть, и целевой сценарий из ТЗ.
...
| Распознавание этикетки и автоматическое открытие карточки | Требует бэкенда и CV-модуля |
| Персональные рекомендации и интерактивный сомелье | Пока не подключены |

В локальный каталог импортировано **1 645 карточек** с описаниями и **1 644 файла фотографий** из переданного датасета. Импорт пропускает неоднозначные соответствия фото и записи, чтобы не показывать неверную бутылку.
...
| [`backend/`](backend/) | Место для серверного API, каталога, авторизации и вызова ML; реализация в работе |
| [`ml/`](ml/) | Место для модели и пайплайна распознавания; реализация в работе |
